...
// 这里用轻量的 advisory lock：直接读，因为 Node.js 单线程下读操作是原子的
// 但为了 API 一致性，还是走锁机制（proper-lockfile 是互斥锁，读用 Stale 选项）
async function withReadLock(fn) {
  await ensureLockDir();
  return lockfile.lock(PROFILES_PATH, { retries: { retries: 5, minTimeout: 50, maxTimeout: 200 }, stale: 5000 })
    .then(async (release) => {
      try {
        const result = await fn();
        return result;
...
async function _getProfilesDecryptedInner() {
  ...
  // 迁移旧格式
  let data = await migrateFormat(raw);
  // 明文加密迁移
...
async function getProfilesDecrypted() {
  return withReadLock(() => _getProfilesDecryptedInner());
}
...
async function checkMismatch() {
  ...
  if (!activeName) {
    return { active: null, mismatch: null };
  }
  const data = await getProfilesDecrypted();
