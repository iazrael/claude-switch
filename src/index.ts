...
// 交互：添加套餐
async function addProfileUI(name?: string): Promise<void> {
  if (!name) {
  ...
  for (const [k, v] of Object.entries(envAnswers) as [string, string][]) {
    if (v.trim()) env[k] = v.trim();
  }
  if (Object.keys(env).length === 0) {
    console.log(chalk.red('至少需要一个环境变量'));
    return;
  }
...
async function removeProfileUI(name?: string): Promise<void> {
  ...
  if (names.length === 0) {
    console.log(chalk.yellow('没有套餐可删除'));
    return;
  }
...
// 交互：切换套餐
async function switchProfileUI(name?: string): Promise<void> {
  const names = await manager.getAllProfileNames();
  if (names.length === 0) {
    console.log(chalk.red('请先添加套餐'));
    return;
  }
...
async function showCurrent(): Promise<void> {
  ...
  if (Object.keys(env).length === 0) {
    console.log(chalk.cyan('当前未配置环境变量'));
    return;
  }
...
async function listProfiles(): Promise<void> {
  ...
  if (names.length === 0) {
    console.log(chalk.yellow('没有已保存的套餐'));
    return;
  }
...
async function firstInstallImport(): Promise<boolean> {
...
async function serveAction(opts: ServeOptions): Promise<void> {
...
program
  ...
  .option('--daemon-child', '内部参数：启动守护子进程', false)
  .action(serveAction);
...
if (process.argv.length === 2 && process.argv[1] === __filename) {
  (async () => {
    try {
      ...
      console.error(chalk.red('操作失败: ' + (err as Error).message));
      process.exit(1);
    }
  })();
} else {
