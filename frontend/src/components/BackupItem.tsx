import styles from '../styles/components/BackupItem.module.css';

interface BackupItemProps {
  fileName: string;
  reason?: string;
  isSelected?: boolean;
  onClick: () => void;
}

export function BackupItem({ fileName, reason, isSelected, onClick }: BackupItemProps) {
  return (
    <div 
      className={`${styles.backupItem} ${isSelected ? styles.backupSelected : ''}`} 
      onClick={onClick}
      style={{ cursor: 'pointer' }}
    >
      <div className={styles.backupLeft}>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, opacity: 0.7 }}>
          <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
          <polyline points="14 2 14 8 20 8" />
          <line x1="16" y1="13" x2="8" y2="13" />
          <line x1="16" y1="17" x2="8" y2="17" />
          <polyline points="10 9 9 9 8 9" />
        </svg>
        <span className={styles.backupName} title={fileName}>{fileName}</span>
        {reason && <span className={styles.backupReason}>{reason}</span>}
      </div>
    </div>
  );
}