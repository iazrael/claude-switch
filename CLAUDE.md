...
lib/
├── config.ts            路径常量（支持 CLAUDE_SWITCH_DIR / CLAUDE_SETTINGS_PATH 环境变量覆盖）
├── crypto-utils.ts      AES-256-CBC 加解密（增强密钥派生：machineId + 硬件特征 + salt）
├── profile-manager.ts   核心业务（套餐 CRUD、切换、迁移、预设模板、备份预览）
├── backup.ts            文件级备份与还原（含原因标注）
├── diff.ts              JSON diff 工具（敏感字段脱敏）
├── logger.ts            按日追加的操作日志
├── serve.ts             守护进程管理（PID 管理、日志轮转、前台/后台启动、停止、状态检查）
├── types.ts             TypeScript 类型定义（ClaudeEnv, Profile, ProfileData, SettingsJson, PresetTemplate, PidInfo, DiffChange）
public/
├── index.html           HTML 结构（含 <link>/<script> 引用）
├── styles.css           CSS 样式（暗色模式，prefers-color-scheme）
└── app.js               前端逻辑（零依赖原生 JS，SPA）
tests/
├── index.test.ts        集成测试（Profile Manager + Crypto + API Endpoints）
└── serve.test.ts        serve 命令测试（PID 管理、端口解析、启动/停止/状态）
env.d.ts                 环境变量类型声明（CLAUDE_SWITCH_DIR, CLAUDE_SETTINGS_PATH, CLAUDE_SWITCH_PORT）
tsconfig.json            TypeScript 配置（strict, NodeNext module）
...
tsx index.ts              # 无参数进入交互式切换
tsx index.ts list         # 列出套餐
tsx index.ts add my-pro   # 添加套餐
tsx server.ts             # 直接启动 Web 服务器
...
tsx index.ts serve                    # 后台启动（daemon）
tsx index.ts serve --foreground       # 前台启动
tsx index.ts serve stop               # 停止服务
tsx index.ts serve status             # 查看状态
```
