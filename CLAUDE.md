...
lib/
├── config.js            路径常量（支持 CLAUDE_SWITCH_DIR / CLAUDE_SETTINGS_PATH 环境变量覆盖）
...
1. **无文件锁**: 多进程/多标签页并发操作 profiles.json 可能导致数据竞争。建议使用 `proper-lockfile` 或类似方案。
2. **Web 端无认证**: 所有 API 端点无认证机制，依赖局域网隔离，不建议公网暴露。
